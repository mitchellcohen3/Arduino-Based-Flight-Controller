...
PIDLoop outer_loop_PID = PIDLoop(dt_outer, MAX_ROLL_LIMIT,-MAX_ROLL_LIMIT, KP_POSITION, KD_POSITION, KI_POSITION, I_MAX_ROLL);

double distTo(double la1, double lo1, double la2, double lo2){
  //earth's radius in meters in montreal, 40 is the height above sea level at mcgill 
  double radius = 6367302 + 40; 
...
double angle(double heading, double x, double y, double x2, double y2){
    ...
    double r = toDeg(tht-heading); //do this to ensure counter clockwise is +ve
...
//Main function for position controller
void position_control(){
  ...
  double current_heading = getHeading(px, py, nx, ny);
  heading_error = angle(current_heading, nx, ny, setpoints[counter].x, setpoints[counter].y);

  desired_roll  = outer_loop_PID.calculate_output(heading_error, yaw_rate);
