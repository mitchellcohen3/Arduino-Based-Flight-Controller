...
void setup() {
    ...
    initialize_interrupts();  //interrupts are used for the Rx/Tx
    //file_num = create_file(); //Create file to store data on SD card

    set_flight_mode("FBW"); //Set the flight mode - options are 'FBW' (Fly-by-wire), 'Autopilot', or 'Stabilize'

    //If the flight mode is set to autopilot, wait until GPS fix is obtained and initialize coordinate system  
    if(flight_mode == 2){
        hold_for_gps_fix();
...
//After cutdown, control glider
    if(cd_ready == 0){
 */     
//If channel 6 on the reciever is off, fly in manual mode
